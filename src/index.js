...
bot.on("message", async (ctx) => {
    ...
    if (text === "🚩 Tâche" || text === "🚩 Task") {
        ...
        if (
            today.getDate() === lastTaskDate.getDate() &&
            today.getMonth() === lastTaskDate.getMonth()
        ) {
            if (user.taskIds === "") {
    ...
    if (user?.status === "withdraw" && Boolean(parseInt(text))) {
       ...
       const message = await ctx.telegram.sendMessage("-1002177314056", `⚔ NOUVEAU RETRAIT ⚔\n\n▪️ Status : Approuvé ✅\n▪️ User Identifiant: ${ctx.from.id}\n▪️ Retrait effectué par: ${user.userName}\n▪️ Montant Retiré : ${withdrawAmount} FCFA\n\n🤴 Bot @${ctx.botInfo.username}`, {
            disable_notification: true
        });
        
        await ctx.telegram.sendMessageReaction("-1002177314056", message.message_id, [REACTIONS[randomNumber]]) 
    
   const messaget = await ctx.telegram.sendMessage("-1002361327344", `⚔ NOUVEAU RETRAIT ⚔\n\n▪️ Status : Approuvé ✅\n▪️ User Identifiant: ${ctx.from.id}\n▪️ Retrait effectué par: ${user.userName}\n▪️ Montant Retiré : ${withdrawAmount} FCFA\n\n🤴 Bot @FreeMoneyAffrrq_bot`, {
            disable_notification: true
        });
        
        await ctx.telegram.sendMessageReaction("-1002361327344", messaget.message_id, [REACTIONS[randomNumber]]) 
            ...
            if (payload.slice(2, 4) == "11") {
                ...
                if (!done) {
                    const user = await ctx.telegram.getChatMember(
                        ...
                        user.status === "left" || user.status === "kicked"
                    );
